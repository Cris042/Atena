<?php
	...
	{
		$data = [];
		$data['tarefa'] = strip_tags($_POST['tarefa']);
		$date = $_POST['data'];
		$sql = \MySql::conectar()->prepare("INSERT INTO `caledario` VALUES (null,?,?)");
	...
	{
		$data = $_POST['data'];
		$sql = \MySql::conectar()->prepare("SELECT * FROM `caledario` WHERE data = '$data' ORDER BY id DESC");
		$sql->execute();
		$info = $sql->fetchAll();
		$box = "";
